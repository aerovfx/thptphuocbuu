...
import { Box, Play, RotateCcw, Activity, Loader2 } from "lucide-react";
...
export default function WFCBuilderPage() {
  ...
  const config = {
    ...
    description: "Tạo nội dung procedural với thuật toán WFC",
  ...
  useEffect(() => {
    ...
    const checkBackend = async () => {
      try {
        const response = await fetch(`http://localhost:8019/api/health`);
        if (response.ok) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="text-center space-y-4">
        <div className="flex items-center justify-center space-x-3">
          ...
          <h1 className="text-3xl font-bold">Wave Function Collapse</h1>
        
              </div>
        <p className="text-lg text-muted-foreground">
          Tạo nội dung procedural với thuật toán WFC
        </p>
