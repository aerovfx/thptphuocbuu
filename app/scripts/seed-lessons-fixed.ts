/**
 * Seed demo lessons for classes - Fixed version
 */
...
async function main() {
  ...
  try {
    ...
    console.log(`📚 Classes with specific lessons: ${classesWithSpecificLessons.length}`)
    console.log(`   Codes: ${classesWithSpecificLessons.map(c => c.code).join(', ')}`)
    
    for (const classItem of classesWithSpecificLessons) {
      const classLessons = lessonsData[classItem.code]
      if (!classLessons) {
        console.log(`⚠️  No lesson data for ${classItem.code}`)
        continue
      }

      console.log(`\n📚 Processing class: ${classItem.name} (${classItem.code})`)
      console.log(`   Will create ${classLessons.length} chapters`)
      ...
      try {
        ...
        console.log(`   ✅ Deleted existing chapters/lessons`)
      } catch (error: any) {
        console.log(`   ⚠️  Could not delete existing: ${error.message}`)
      }
    ...
    console.log(`\n📚 Creating generic lessons for ${otherClasses.length} other classes`)
    ...
    for (const classItem of otherClasses) {
      ...
      if (existingChapters.length > 0) {
        console.log(`  ⏭️  Skipped (already has ${existingChapters.length} chapters)`)
        continue
      }
