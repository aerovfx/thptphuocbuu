...
class AuthService {
  ...
  static Future<Map<String, dynamic>> signInWithGoogle() async {
    try {
      ...
      if (response.statusCode == 200) {
        if (data['success'] == true) {
          ...
          if (userData != null) {
            ...
            return {
              'success': false,
              'error': 'Server không trả về thông tin người dùng',
            };
          }
        } else {
          final errorMsg = data['error'] as String? ?? 'Đăng nhập Google thất bại';
          print('[AuthService] Google login failed: $errorMsg');
          return {
            'success': false,
            'error': errorMsg,
          };
        ...
        final errorMsg = data['error'] as String? ?? 'Xác thực Google thất bại';
      ...
      print('[AuthService] Google Sign-In unexpected error: $e');
  ...
  static Future<User?> getCurrentUser() async {
    try {
      print('[AuthService] Checking current session health...');
      ...
      if (response.statusCode == 200) {
        ...
        print('[AuthService] Session invalid or expired: ${response.statusCode}');
        // ApiService already clears storage on 401/403
        return null;
      }
      return null;
    } catch (e) {
      print('[AuthService] Error checking session: $e');
      return null;
    }
  }

  // Quick session check for startup
